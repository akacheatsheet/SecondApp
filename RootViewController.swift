...
class RootViewController: UIViewController {
	...
	func addScoreDiff(scoreInc: Int) {
		self.scoreInced += scoreInc
	...
	func addScoreFile(scoreInc: Int) {
		let newScore = self.restoreScoreFile() + scoreInc
	...
	@objc func handleSingleTap(_ sender: UITapGestureRecognizer) {
		if (sender.state == UIGestureRecognizer.State.ended) {
			// ナマ
			self.addScoreRaw(scoreInc: self.scoreInc)
			
			// 差分
			self.addScoreDiff(scoreInc: self.scoreInc)
			
			// ファイル
			self.addScoreFile(scoreInc: self.scoreInc)
			
			// xor
			self.addScoreXor(scoreInc: self.scoreInc)
		}
	}
